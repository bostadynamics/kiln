...
def main() -> None:
    ...
    print(f"Reading Pattern 0 from Address {hex(START_ADDR)}...")
    print("-" * 30)
    print(f"{'Step':<6} | {'Temp (°C)':<10} | {'Time (min)':<10}")
    print("-" * 30)
    current_pattern = inst.get_current_program()
    current_step = inst.get_current_step()
    min_left = inst.get_current_step_left_min()
    sec_left = inst.get_current_step_left_sec()
    print(f"Reading Pattern {current_pattern} from Address...")
    print("-" * 30)
    print(f"{'Step':<6} | {'Temp (°C)':<10} | {'Time (min)':<10}")
    print("-" * 30)
    for step, (temp, time) in enumerate(inst.get_pattern(current_pattern)):
        print(
            f"{step:<6} | {temp:<10} | {time:<10} {'<<<<<' if step == current_step else ''}"
        )
    print(
        f"Current temp: {inst.get_pv()}, Target temp: {inst.get_setpoint()}, time left: min - {min_left} sec - {sec_left}"
    )
    # for step in range(10 * 10):
    #     # Calculate address for this step
    #     # Step 0 = 2000, Step 1 = 2002, Step 2 = 2004, etc.
    #     addr = START_ADDR + (step)

    #     # Read 2 registers: [Temperature, Time]
    #     # functioncode=3 means "Read Holding Registers"
    #     try:
    #         data = inst.read_registers(addr, 1, functioncode=3)
    #     except Exception as e:
    #         # print(f"{hex(addr):<6} | {step:<6} | {'nan':<10} | {'nan':<10}")
    #         continue
    #     temp_raw = data[0]
    #     # data = inst.read_registers(addr - (8 * 8), 1, functioncode=3)

    #     time_val = data[0]
    #     # time_val = data[1]

    #     # Convert raw temp (usually x10)
    #     temp_c = temp_raw / 10.0

    #     print(f"{hex(addr):<6} | {step:<6} | {temp_c:<10} | {time_val:<10}")
